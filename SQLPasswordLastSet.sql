...
,sp.[principal_id] as "Account Principal ID"
	  ...
	  --,LOGINPROPERTY(sp.name,'BadPasswordCount') as "Account BadPasswordCounte"
	  --,LOGINPROPERTY(sp.name,'IsLocked') as "Account IsLocked"
	  --,LOGINPROPERTY(sp.name,'LockoutTime') as "Account LockoutTime"
      ...
      --, case when (PWDCOMPARE('',sp.password_hash)=1) then 'Yes' Else 'No' End as "Blank Password?"
       ...
       on (sp.sid = sl.sid)
	   where sp.name = 'zabbix_mon_prd'
	   ORDER BY LOGINPROPERTY(sp.name,'PasswordLastSetTime') DESC;